...
void GfxInfo::GetData() {
 ...
 gfxCriticalNote << "stringToFill: " << stringToFill "\n";
  ...
  // override render devide from DRI_PRIME
  const char* driPrime = PR_GetEnv("DRI_PRIME");
  if (driPrime) {
    mDrmRenderDevice.Assign(driPrime);
    gfxCriticalNote << "assigning GPU according to DRI_PRIME: " << driPrime
                    << "\n";
  }
  // hardcode 3090:
//  mVendorId.AssignLiteral("0x10de");
//  mDeviceId.AssignLiteral("0x2204");
  ...
  // print out chosen gpu and its glVendor data
  gfxCriticalNote << "\nchose GPU: " << mVendorId.get() << "/" << mDeviceId.get() << "\n" << "GL_VENDOR: " << glVendor.get() << "\n" << "GL_RENDERER: " << glRenderer.get() << "\n" << "GL_VERSION: " << glVersion.get() << "\n" ;
  // print out all of pciVendors:

  for (size_t i = 0; i < pciVendors.Length(); ++i) {
    gfxCriticalNote << "PCI candidate was " << pciVendors[i].get() << "/"
                    << pciDevices[i].get() << "\n";
  }
...
// TODO: rewrite GfxInfo::GetData() by splitting it into many smaller methods
