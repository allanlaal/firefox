...
void GetDualGPUInfo(SystemInfo *info)
{
    ...
    // get active GPU from DRI_PRIME
    std::string dri_prime = GetEnvironmentVar("DRI_PRIME");
    // if numeric, use it as the active GPU index
    if (!dri_prime.empty()) {
      if (isdigit(dri_prime)) {
        info->activeGPUIndex = std::stoi(dri_prime);
        return;
      }
    ...
    info->activeGPUIndex = 2; // TODO: REMOVE !!!
}
int AssumeGPUIndex(SystemInfo *info) {
  // On dual-GPU systems we assume the non-Intel GPU is the graphics one.
  // TODO: this is incorrect and problematic.  activeGPUIndex must be removed if it cannot be determined correctly.  A potential solution is to create an OpenGL context and parse GL_VENDOR.  Currently, our test infrastructure is relying on this information and incorrectly applies test expectations on dual-GPU systems when the Intel GPU is active. http://anglebug.com/6174.
  int active = 0;
  bool hasIntel = false;
